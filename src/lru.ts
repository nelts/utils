...
export default class LRU<T = any> {
  private max: number;
  private size: number;
  private cache: Map<T, LRU_VALUE>;
  private _cache: Map<T, LRU_VALUE>;
  ...
  set(key: T, value: any, options?: LRU_OPTIONS) {
    const maxAge = options && options.maxAge;
  ...
  keys() {
    ...
    function checkEntry(entry: [T, LRU_VALUE]) {
      const key = entry[0];
      const item = entry[1];
    ...
    return Array.from<T>(cacheKeys.keys());
  }

  private _update(key: T, item: LRU_VALUE) {
